...
以下內容皆可於此 Repository 的 Markdown 文件中瀏覽：

| 模組名稱 | 說明 |
|----------|------|
| ✅ 取得病患基本資料 | 使用 jQuery + AJAX 從後端取得病患資訊，自動填入表單 |
| ✅ 依參數讀取 SMB 檔案 | 根據前端參數拼出 PDF/Excel 路徑，從共享磁碟下載檔案 |
| ✅ HTML 轉 PDF 並儲存 | 使用 iText 套件將記錄表單輸出 PDF 並儲存至網路路徑 |
| ✅ 醫療影像上傳批次維護 | 處理舊系統資料缺漏問題，改善合格率並加強 DB 資源釋放 |
| ✅ 語音播放（TTS 整合） | 串接 Google Cloud Speech，語音合成並播放於前端 |
| ✅ 檔案完整性校驗 | 使用 Shell Script 定期產生/比對 MD5，異常時自動還原 |
| ✅ Log 回應時間統計工具 | 使用 Java 統計 Tomcat Log 檔，計算平均與3秒內比率 |
